...
RSpec.describe Package do
  describe '.create_from_list' do
    ...
    it 'saves the correct r_version_required from various formats' do
      various_dependency_formats = ['Rglpk,rgl,corrplot,lattice,R (>= 2.10)', 'R(>= 2.10.1)', 'R (>= 3.5.0),',
                                    'tuneR (>= 1.0), R (>= 2.10)', nil, 'corrplot']
  ...
  describe '#update_from_description' do
    it 'updates a package with the correct attributes from a decompressed, DCF-formatted file' do
      ...
      package.update_from_description(File.read(Rails.root.join('spec', 'fixtures',
                                                                'decompressed_packages_description_example')))
